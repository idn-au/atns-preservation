"""Recover ATNS subcategory definitions from pinned Wayback captures."""
...
MANIFEST = ROOT / "reports" / "subcategories.tsv"
OUTPUT = ROOT / "specs" / "sub-categories-definitions.tsv"
...
ID_PATTERN = re.compile(r"[?&]subcategoryid=(\d+)", re.IGNORECASE)
...
def load_subcategories(path: Path) -> list[Subcategory]:
    subcategories: list[Subcategory] = []
    with path.open(encoding="utf-8", newline="") as source:
        for row in csv.reader(source, delimiter="\t"):
            if len(row) != 3:
                raise SystemExit(f"Unexpected sub-category row: {row!r}")
...
def load_captures() -> dict[str, list[Capture]]:
    query = urlencode(
        {
            "url": "atns.net.au/subcategory.asp?subcategoryid=*",
            "output": "json",
...
def clean_fragment(fragment: str) -> str:
    value = " ".join(html.unescape(TAG_PATTERN.sub(" ", fragment)).split())
    for suffix in NON_DEFINITION_SUFFIXES:
...
def recover_subcategory(
    subcategory: Subcategory, captures: list[Capture]
) -> dict[str, str | int]:
    ...
    matching = [
        ...
        if item[1].casefold() == subcategory.label.casefold()
    ]
    ...
    return {
        "concept_iri": subcategory.iri,
        "notation": subcategory.notation,
        "pref_label": subcategory.label,
...
def main() -> None:
    ...
    subcategories = load_subcategories(args.manifest)
    ...
    if args.retry_unresolved:
        ...
        subcategories_to_crawl = [
            subcategory
            for subcategory in subcategories
            if subcategory.notation not in rows_by_notation
            or str(rows_by_notation[subcategory.notation].get("review_status", ""))
            in retry_statuses
        ]
    else:
        subcategories_to_crawl = subcategories
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(
                recover_subcategory,
                subcategory,
                captures.get(subcategory.notation, []),
            ): subcategory
            for subcategory in subcategories_to_crawl
        }
        for future in as_completed(futures):
            subcategory = futures[future]
            rows_by_notation[subcategory.notation] = future.result()
            print(
                f"{subcategory.notation}: "
                f"{rows_by_notation[subcategory.notation]['review_status']}",
                flush=True,
            )

    rows = [rows_by_notation[subcategory.notation] for subcategory in subcategories]
