...
The profile definitions in `prezconfig/custom_profiles.ttl` are local test mirrors. The deployment source of truth is `prez-profiles.trig` in the `idn-prez4` repository; deployment changes are made there first and then synchronised back here. The current profiles retain the separate ATNS CreativeWork and related ODRL Agreement pattern. They do not yet implement the provisional dual-`rdf:type` design described below.
