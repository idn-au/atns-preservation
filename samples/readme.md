...
The sample keeps the original ATNS pattern of treating `Entity` as the central record. Every row preserved from the source `Entities` table is explicitly typed `atns:Entity`. Agreement-category records are also typed `atns:AgreementRecord`, a subclass of `atns:Entity`, while retaining the original `atns:category` concept. This ATNS record type does not entail `odrl:Agreement`. Other ATNS entity category and subcategory values remain SKOS concepts from `vocabs/`. Where a source category maps cleanly to a published class, that class is asserted in addition; for example, Organisation-category records in this sample are also typed `schema:Organization`.
