...
The sample keeps the original ATNS pattern of treating `Entity` as the central record. Every row preserved from the source `Entities` table is explicitly typed `atns:Entity`. The ATNS entity category and subcategory values remain SKOS concepts from `vocabs/`, rather than being replaced by a new custom class hierarchy. Where a source category maps cleanly to a published class, that class is asserted in addition; for example, Organisation-category records in this sample are also typed `schema:Organization`. Agreement-category records are identified by their `atns:category` concept rather than by a redundant custom agreement class.
