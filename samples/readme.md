...
The sample keeps the original ATNS pattern of treating `Entity` as the central record. Every row preserved from the source `Entities` table is explicitly typed `atns:Entity`. Agreement-category records are also typed `schema:CreativeWork` and soft-typed `catobjtyp:Agreement` using `schema:additionalType`, while retaining the original `atns:category` concept. This publication classification does not entail `odrl:Agreement`. Every `atns:Reference` instance is also typed `schema:CreativeWork` and is not typed `dcat:Resource`. Other ATNS entity category and subcategory values remain SKOS concepts from `vocabs/`. Where a source category maps cleanly to a published class, that class is asserted in addition; for example, Organisation-category records in this sample are also typed `schema:Organization`.
...
Instance data uses flat IDN PID-style resource IRIs under `https://data.idnau.org/pid/resource/`. ATNS entities, references, relationship rows and the sample dataset share this resource namespace and are distinguished by their RDF types rather than by type-specific IRI paths. IRI suffixes use globally distinct deterministic UUIDs, such as `idn-resource:34524ede-2abc-50f1-af30-8493753c928e`, rather than source database identifiers. Do not duplicate these HTTP PID IRIs as `schema:identifier` nodes.

Legacy ATNS identifiers are preserved separately as source-system token values. For example, the public EID `A003259` is recorded with `atns:eid "A003259"^^xsd:token`, the source `Entities.EntityID` value `24` is recorded with `atns:sourceEntityId "24"^^xsd:token`, source `Refs.RefID` values are recorded with `atns:sourceReferenceId`, and source `Entity_Entity.Entity_EntityID` values are recorded with `atns:sourceRelationshipId`. The deleted flag is expressed explicitly as an `xsd:boolean`. Use `schema:identifier "..."^^xsd:token` only if a later sample needs a non-HTTP catalogue identifier that is not already covered by one of these preservation properties. The `https://data.idnau.org/pid/org/` and `https://data.idnau.org/pid/person/` bases are reserved for later first-class organisation and person identifiers, rather than being used for every legacy ATNS entity row that happens to describe an organisation or person.
